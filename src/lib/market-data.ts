...
const NSE_ALIASES: Record<string, string[]> = {
  nifty50: ["NIFTY 50"], niftynext50: ["NIFTY NEXT 50"], niftymidcap150: ["NIFTY MIDCAP 150"], niftysmallcap250: ["NIFTY SMALLCAP 250"], nifty500: ["NIFTY 500"], niftybank: ["NIFTY BANK"], niftyfin: ["NIFTY FINANCIAL SERVICES"], niftyit: ["NIFTY IT"], niftypharma: ["NIFTY PHARMA"], niftyauto: ["NIFTY AUTO"], niftyfmcg: ["NIFTY FMCG"], niftymetal: ["NIFTY METAL"], niftyhealthcare: ["NIFTY HEALTHCARE"], niftyinfra: ["NIFTY INFRASTRUCTURE"], niftyrealty: ["NIFTY REALTY"],
};
...
export function getMarketStatus(): { status: string; nseStatus: string; bseStatus: string } {
  const parts = new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", weekday: "short", hourCycle: "h23" }).formatToParts(new Date()); const get = (type: string) => parts.find(p => p.type === type)?.value ?? "0"; const day = get("weekday"), minutes = Number(get("hour")) * 60 + Number(get("minute"));
  if (day === "Sat" || day === "Sun") return { status: "CLOSED", nseStatus: "Closed", bseStatus: "Closed" }; if (minutes < 555) return { status: "PRE-OPEN", nseStatus: "Pre-Open", bseStatus: "Pre-Open" }; if (minutes <= 930) return { status: "OPEN", nseStatus: "Trading", bseStatus: "Trading" }; return { status: "CLOSED", nseStatus: "Closed", bseStatus: "Closed" };
}
