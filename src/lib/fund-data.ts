...
// NAVs are published daily, so keep the cache short enough that the terminal
// picks up a newly published NAV without repeatedly hammering MFAPI.
...
async function searchScheme(fundName: string): Promise<SearchItem> {
  ...
  const exactDirectGrowth = items.find(x => norm(x.schemeName) === target && /direct/i.test(x.schemeName) && /growth/i.test(x.schemeName));
  ...
  return exactDirectGrowth ?? exact ?? directGrowth ?? items[0];
}
