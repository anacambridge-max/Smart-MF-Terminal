...
function OpportunityCard({ item, rank, onClick }: { item: ScoredSector; rank: number; onClick: () => void }) {
  return (
    <button onClick={onClick} className="glass-card rounded-xl p-4 text-left w-full group hover:border-blue-500/40 transition-all hover:-translate-y-0.5">
      ...
      <div className="mt-4 pt-3 border-t border-terminal-border flex items-end justify-between gap-3"><div className="min-w-0"><p className="text-[8px] uppercase tracking-wider text-terminal-text-muted">Opportunity Score</p><p className="text-xl font-bold font-mono text-blue-400">{item.opportunityScore}<span className="text-[10px] text-terminal-text-muted">/100</span></p></div><div className="text-right min-w-0"><p className="text-[8px] uppercase tracking-wider text-terminal-text-muted">MF Match</p><p className="text-[9px] text-blue-400 truncate max-w-[170px]">{item.recommendedFund}</p></div></div>
    </button>
  );
}
  ...
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-3 mb-3 px-1">
        ...
        <div className="text-[9px] text-terminal-text-muted uppercase tracking-wider">Top 5 opportunities · validated market snapshot</div>
      </div>
