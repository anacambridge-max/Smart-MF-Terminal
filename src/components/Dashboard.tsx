...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-terminal-bg terminal-grid">
      ...
      <nav className="sticky top-0 z-40 border-b border-terminal-border bg-[#0a1019]/95 backdrop-blur-xl">
        <div className="max-w-[1600px] mx-auto px-4 lg:px-6">
          <button className="md:hidden w-full py-3 text-xs font-semibold text-terminal-text flex items-center justify-between" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}><span>{TABS.find(t => t.key === activeTab)?.icon} {TABS.find(t => t.key === activeTab)?.label}</span><span className="text-terminal-text-muted">{mobileMenuOpen ? "▲" : "▼"}</span></button>
          <div className={`${mobileMenuOpen ? "flex" : "hidden"} md:flex overflow-x-auto`}>
            {TABS.map(tab => (
              <button key={tab.key} onClick={() => { setActiveTab(tab.key); setMobileMenuOpen(false); }} className={`group relative flex items-center gap-2 px-4 lg:px-5 py-3.5 text-[11px] font-semibold whitespace-nowrap transition-colors border-b-2 ${activeTab === tab.key ? "border-blue-500 text-blue-400 bg-blue-500/[.035]" : "border-transparent text-terminal-text-muted hover:text-terminal-text-dim"}`}>
                <span className="text-sm">{tab.icon}</span><span>{tab.label}</span><span className="hidden xl:inline text-[8px] uppercase tracking-wider opacity-40">{tab.hint}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <main className="max-w-[1600px] mx-auto px-4 lg:px-6 py-5">
        {activeTab === "dashboard" && <div className="space-y-5 fade-in"><HeroOpportunity data={data} onSectorClick={handleSectorClick} /><div className="grid grid-cols-1 xl:grid-cols-3 gap-5"><div className="xl:col-span-2"><TopLosersTable losers={data.topLosers} onSectorClick={handleSectorClick} compact /></div><SectorHeatmap sectors={data.sectors} onSectorClick={handleSectorClick} compact /></div><DecisionEngine data={data} onSectorClick={handleSectorClick} compact /></div>}
      ...
      <footer className="border-t border-terminal-border mt-8 py-4"><div className="max-w-[1600px] mx-auto px-4 text-center text-[9px] uppercase tracking-wider text-terminal-text-muted">Smart MF Terminal · NSE live index data · Historical technical analysis · Decision support only · Not financial advice</div></footer>
    </div>
  );
}
